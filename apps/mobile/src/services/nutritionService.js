// apps/mobile/src/services/nutritionService.js
...
export const nutritionService = {
  ...
  async getDailyMicronutrients(userId, date) {
    try {
      ...
      // Buscar refeições consumidas do dia
      const { data: meals, error: mealsError } = await supabase
        .from('meals')
        .select('id, ingredients, consumed')
        .eq('user_id', userId)
        .eq('meal_date', date)
        .eq('consumed', true);
      ...
      // Somar micronutrientes de todos os ingredientes
